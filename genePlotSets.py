...
import ROOT
from ROOT import TCanvas, TFile, TProfile, TNtuple, TH1F, TH2F, TGraphErrors, TGraphMultiErrors
import argparse
import ctypes
import os
import gc
...
def overwrightSomePlots(hBlank, lHists, wErr, legALICE,legJet,leg,\
    ...
    hBlank.Draw('same')
    drawOption = addDrawOption + 'same'
    for hBin in range(0, len(lHists)):
        color = colorList[hBin]
        style = styleList[hBin]
        h = lHists[hBin]
        setHistLooks(h, style, color, wErr)
        h.Draw(drawOption)
...
def overwrightSomePlotsWithErr(hBlank, lHists, lHErr, legALICE,legJet,leg,\
    ...
    for hBin in range(0, len(lHists)):
        ...
        style = styleList[hBin]
        hVal = lHists[hBin]
        setHistLooks(hVal, style, color, 0)
        hErr = lHErr[hBin]
...
# == s == Make Some Hists        #############################################
def addSomeHistsLegend(leg, lHists, lHistTags, textSize):
    for histKinds in range(0,len(lHists)):
        leg.AddEntry(lHists[histKinds], lHistTags[histKinds], "plf")
...
def setHistLooks(h, style, color, wErr):
    ...
    if wErr==1:
        # h.GetAttLine(1).SetLineColor(color)
        h.GetAttFill(1).SetFillStyle(3001)
        h.GetAttFill(1).SetFillColorAlpha(color, 5.0)
        h.GetAttFill(1).SetFillColor(color)
        

def setHistLooksWSysE(h, hErr, style, color, wErr):
    h.SetLineColor(color)
    h.SetLineWidth(4)
    h.SetMarkerColor(color)
    h.SetMarkerStyle(style)
    ...
    hErr.SetMarkerColor(color)
    hErr.SetLineColor(color)
    hErr.SetLineWidth(2)
    hErr.SetFillStyle(3001)
    hErr.SetFillColorAlpha(color, 5.0)
    hErr.SetFillColor(color)
    hErr.SetMarkerSize(1.5)
    hErr.SetMarkerStyle(style)
...
# == s == Set Axis histgram Setting  ###########################################
def makeBlank1DHist(nBin, lRange, lTitle):
    title = lTitle[0]
    xmin = lRange[0][0]
    xmax = lRange[0][1]
    hBlank = ROOT.TH1D(title, title, int(nBin), xmin, xmax)
    # hBlank = ROOT.TH1D(lTitle[0], lTitle[0], nBin, lRange[0][0], lRange[0][1])
    hBlank.SetXTitle(lTitle[1])
    hBlank.GetXaxis().SetTitleOffset(1.2)
    hBlank.GetXaxis().SetTitleSize(0.045)
    hBlank.GetXaxis().SetLabelSize(0.045)
    hBlank.SetYTitle(lTitle[2])
    hBlank.GetYaxis().SetTitleOffset(1.2)
    hBlank.GetYaxis().SetTitleSize(0.045)
    hBlank.GetYaxis().SetLabelSize(0.045)
    hBlank.GetYaxis().SetRangeUser(lRange[1][0], lRange[1][1])

    return hBlank
# == e == Set Axis histgram Setting  ###########################################
